  ...
  return (
    <>
            ...
            <form onSubmit={handleRegister}>
                ...
                <label className={`2px ${agree ? "text" : "text-danger"}`} htmlFor="terms">Accept Smart Home Terms and Conditions</label>
                </div>
                <input disabled={!agree} className='bg-primary rounded w-50 mx-auto border-0 text-white mt-2' type="submit" value="Register" />
            </form>
            <p>Already have an account?<Link to={"/login"} onClick={navigateLogin} className="text-primary pe-auto text-decoration-none">Please Sign up</Link></p>
        </div>
    );
    </>
    );
